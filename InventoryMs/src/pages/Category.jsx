...
function Category() {
  ...
  const getProductsByCategory = (categoryName) => {
    return filteredData.filter(item => item.name.includes(categoryName));
  };
  ...
  // (no-op) transparent image removed; categories render without image column

  // Categories Data - using actual product data from table 2
  ...
  // Table 3 Data (no headers) - reuse values from table 1, rename categories
  const table3DataNames = ["Cardigans", "Hoodies", "Bracelets", "Blouses"];
  const table3Data = table3DataNames.map((name, idx) => {
    const src = categoriesData[idx % categoriesData.length];
    return {
      category: name,
      stock: src.stock,
      price: src.price,
      ...
      profitability: src.profitability,
      impressionScore: src.impressionScore,
    };
  });

  // Merge categories into a single list for one unified table
  const combinedCategories = [...categoriesData, ...table3Data];
  ...
  const data = [
    ...
    // Jeans products
    {
      ...
      purchases: 145,
      impressions: 3450,
      profitability: "88 - Very High",
      impressionScore: "95 - Highest",
      image: "/src/images/rio_2_movie_2-wallpaper-1366x768.jpg",
      hasSubItems: false,
      isActive: true,
    },
    {
      ...
      purchases: 98,
      impressions: 1750,
      profitability: "92 - Very High",
      impressionScore: "78 - High",
      image: "/src/images/rio_2_movie_2-wallpaper-1366x768.jpg",
      hasSubItems: false,
      isActive: true,
    },
    {
      ...
      purchases: 112,
      impressions: 2310,
      profitability: "75 - High",
      impressionScore: "82 - High",
      image: "/src/images/rio_2_movie_2-wallpaper-1366x768.jpg",
      hasSubItems: false,
      isActive: true,
    },
  ...
  // Categories that are exempt from active/inactive filtering
  const exemptedCategories = ["Jeans", "Dresses", "Skirts", "T-Shirts", "Cardigans", "Hoodies", "Bracelets", "Blouses"];
  
  // Always show all products (categories are exempted from filtering)
  const filteredData = data;

  const renderCategoryRow = (item, index, isTable3 = false) => {
      ...
      >
        ...
        <td className="p-1 w-8">
          <button 
            ...
            title="Click to expand/collapse products"
          >
        ...
        <td className="p-0 w-48">
          <div className="pl-1 pr-2 py-2">
            <div className="font-medium text-xs text-gray-900 cursor-pointer hover:text-blue-600">
              {item.name}
            </div>
            {showKPI.has(index) && (
              <button 
        ...
        <td className={`${isTable3 ? 'pl-0 pr-1 py-2' : 'p-2'} w-73 bg-blue-200 bg-opacity-50 hover:bg-blue-300 hover:bg-opacity-70`}>
        ...
        <td className={`${isTable3 ? 'pl-0 pr-1 py-2' : 'p-2'} w-32 bg-blue-200 bg-opacity-50 hover:bg-blue-300 hover:bg-opacity-70`}>
          <div className="flex items-center space-x-1">
  ...
  const renderRow = (item, index, isSubItem = false) => (
    <>
      ...
      >
        ...
        {/* Within Shop (duplicate profitability for now) */}
        <td className="p-2 w-32 bg-blue-200 bg-opacity-50 hover:bg-blue-300 hover:bg-opacity-70">
          <div className="flex items-center space-x-1">
            <span className="text-gray-700 text-xs font-semibold">{item.profitability.split(' - ')[0]}</span>
            <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium border ${getScoreCardStyle(item.profitability)}`}>
    ...
    <div className={`min-h-screen overflow-x-auto ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
      ...
      {/* Categories rows injected into main table below using unified table */}

      {/* Categories Table */}
      <div className="bg-white rounded-lg shadow-sm overflow-hidden relative mb-0">
        <div className="overflow-x-auto">
          <table className="w-full min-w-full">
            <thead className="border-b border-gray-300">
              {/* First row - Group headers */}
              <tr>
                <th rowSpan="2" className="p-1 text-left text-xs font-medium text-gray-900 bg-gray-100 w-8">
                  <div className="flex items-center space-x-1">
                    <span></span>
                  </div>
                </th>
                <th rowSpan="2" className="p-0 text-left text-xs font-medium text-gray-900 bg-gray-100 w-48">
                  <div className="pl-0 pr-2 py-3 flex items-start space-x-1">
                ...
                <th rowSpan="2" className="px-1 py-3 text-left text-xs font-medium text-gray-900 bg-blue-200 bg-opacity-50 w-95">
            ...
            <tbody>
              {/* Map categories to the table columns */}
              {combinedCategories.map((cat, idx) => {
                const categoryRow = {
                  ...
                  impressionScore: cat.impressionScore,
                  image: '',
                  hasSubItems: false,
                  isActive: true,
                };
                // Only show products table for T-Shirts and Jeans
                const showProductsTable = cat.category === "T-Shirts" || cat.category === "Jeans";
                
                return (
                  <React.Fragment key={`category-wrapper-${idx}`}>
                    {renderCategoryRow(categoryRow, `cat-${idx}`)}
                    {expandedCategories.has(cat.category) && showProductsTable && (
                      <tr>
                        <td colSpan="11" className="p-0">
                          <table className="w-full">
                            <thead>
                              {/* First row - Group headers */}
                              <tr>
                                <th rowSpan="2" className="p-1 text-left text-xs font-medium text-gray-900 bg-gray-100 w-8"></th>
                                <th rowSpan="2" className="px-2 py-3 text-left text-xs font-medium text-gray-900 bg-gray-100 w-20">Image</th>
                                <th rowSpan="2" className="px-2 py-3 text-left text-xs font-medium text-gray-900 bg-gray-100 w-48">Product Name</th>
                                <th rowSpan="2" className="px-2 py-3 text-left text-xs font-medium text-gray-900 bg-gray-100 w-16">Stock</th>
                                <th rowSpan="2" className="px-2 py-3 text-left text-xs font-medium text-gray-900 bg-gray-100 w-20">Price</th>
                                <th rowSpan="2" className="px-2 py-3 text-left text-xs font-medium text-gray-900 bg-gray-100 w-16">Status</th>
                                <th colSpan="3" className="px-2 py-2 text-center text-sm font-bold text-black bg-gray-200"></th>
                                <th rowSpan="2" className="px-1 py-3 text-left text-xs font-medium text-gray-900 bg-blue-200 bg-opacity-50 w-40">within Category</th>
                                <th rowSpan="2" className="px-0 py-3 text-left text-xs font-medium text-gray-900 bg-blue-200 bg-opacity-50 min-w-[160px] w-35">within Shop</th>
                                <th rowSpan="2" className="px-0 py-3 text-left text-xs font-medium text-gray-900 bg-blue-200 bg-opacity-50 min-w-[160px] w-35">7 Days Impression Score</th>
                              </tr>
                              {/* Second row - Individual column headers */}
                              <tr>
                                <th className="px-2 py-3 text-left text-xs font-medium text-black bg-gray-200 w-16 pt-0 pb-4">Rate</th>
                                <th className="px-2 py-3 text-left text-xs font-medium text-black bg-gray-200 w-20 pt-0 pb-4">Purchases</th>
                                <th className="px-2 py-3 text-left text-xs font-medium text-black bg-gray-200 w-20 pt-0 pb-4">Impressions</th>
                              </tr>
                            </thead>
                            <tbody>
                              {getProductsByCategory(cat.category).map((product) => {
                                const globalIndex = data.findIndex(p => p.id === product.id);
                                return renderRow(product, globalIndex);
                              })}
                            </tbody>
                          </table>
                        </td>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2 text-gray-700 px-3 py-2">
                    <span className="text-sm font-medium">Purchase Rate</span>
